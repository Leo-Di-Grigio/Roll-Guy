package game.state.location.lighting;
...
public class LocationLighting {
	...
	public static void removeArea(Location loc, int key){
		if(loc != null){
			loc.getLight().lightingAreas.remove(key);
			loc.requestUpdate();
		}
	}
	...
	public static void addArea(Location loc, int key, int x0, int y0, int x1, int y1, int power){
		if(loc != null){
			...
			// add
			loc.getLight().lightingAreas.put(key, new LightingArea(x0, y0, x1, y1, power));
		}
	}
	...
	public static void printAreas(Location loc){
		if(loc != null){
			...
			// all areas
			Set<Integer> keys = loc.getLight().lightingAreas.keySet();
			for(Integer key: keys){
				LightingArea area = loc.getLight().lightingAreas.get(key);
				Log.msg("ID: " + key 
	...
	public static void updateLighting(Location loc){
		if(loc != null){
			...
			// lighting areas
			for(LightingArea area: loc.getLight().lightingAreas.values()){
